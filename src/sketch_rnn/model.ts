...
export class SketchRNN {
  ...
  async initialize() {
    ...
    this.encoder = await tfl.loadLayersModel(this.checkpointURL.concat('/encoder_model/model.json'));
    console.log('Initialized encoder');
    
    this.z_to_init = await tfl.loadLayersModel(this.checkpointURL.concat('/z_to_init_model/model.json'));
    console.log('Initialized z_to_init');
    
    this.decoder = await tfl.loadLayersModel(this.checkpointURL.concat('/decoder_model/model.json'));
    ...
    var imgDataReq = await fetch('https://storage.googleapis.com/clouddatawu/dog/image/529.json');
  ...
  dispose() {
     ...
     if (this.decoder) {
      ...
      console.log('disposed decoder');
    }

     if (this.z_to_init) {
      this.z_to_init.dispose();
      this.z_to_init = undefined;
      console.log('disposed z_to_init');
    }
